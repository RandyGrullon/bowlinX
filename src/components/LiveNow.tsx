...
const primary =
  'inline-flex h-10 flex-1 items-center justify-center gap-2 rounded-xl bg-accent px-4 text-sm font-semibold text-accent-fg shadow-sm transition hover:brightness-110 active:scale-[0.98]';
const secondary =
  'inline-flex h-10 flex-1 items-center justify-center gap-2 rounded-xl border border-line bg-surface px-4 text-sm font-medium text-fg transition hover:bg-surface-2 active:scale-[0.98]';

/**
 * "En juego ahora": los eventos de tus ligas que se están jugando (desde 30 minutos antes de la hora
 * de la liga hasta la medianoche), con acceso directo para anotar. Con `lid`, solo los de esa liga.
 */
export function LiveNow({ lid }: { lid?: string }) {
  const { feeds, leagues } = useNotifications();
  const now = useNow();
  const games = liveGames(lid ? feeds.filter((f) => f.lid === lid) : feeds, leagues, now);
  if (!games.length) return null;
  return (
    <section className="flex flex-col gap-2" aria-label="En juego ahora">
      {games.map((g) => (
        <LiveCard key={`${g.feed.lid}:${g.event.id}`} game={g} showLeague={!lid} />
      ))}
    </section>
  );
}

function LiveCard({ game, showLeague }: { game: LiveGame; showLeague: boolean }) {
      ...
      <div className="flex flex-col gap-3 p-4">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            {showLeague && league.kind !== 'torneo' && <p className="truncate text-xs font-medium text-muted">{league.name}</p>}
